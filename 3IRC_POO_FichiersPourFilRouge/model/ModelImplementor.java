...
public class ModelImplementor {
	...
	public PieceSquareColor getPieceColor(Coord coord) {
		...
		var tmp = this.findPiece(coord);
		if(tmp != null) {			
			color = tmp.getPieceColor();	
		}
		 return color;
	}
	...
	public boolean isPiecehere(Coord coord) {
		...
		isPiecehere = this.findPiece(coord) == null ? false : true;
	...
	public boolean isMovePieceOk(Coord initCoord, Coord targetCoord, boolean isPieceToTake) {
		...
		if(this.isPiecehere(initCoord) && !this.isPiecehere(targetCoord)) {
			PieceModel pieceModel = this.findPiece(initCoord);
	 ...
	 */
	 private PieceModel findPiece(Coord coord) {		// TODO : mettre en "private" apr�s test unitaires
		 
		PieceModel findPiece = null;

		for (PieceModel piece : pieces) {
			if(coord.getLigne() == piece.getLigne() && coord.getColonne() == piece.getColonne()) {
				return piece;
			}
		}
		return null;
